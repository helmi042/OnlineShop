...
$data = ambilData($query, $id_produk);
$data = $data[0];
...
if(isset($_POST['simpan'])){
  ...
  if($typeFoto == 'image/jpg' || $typeFoto == 'image/jpeg' || $typeFoto == 'image/png'){
    if($ukuranFoto > 1000000){
      ...
      saveData($query);
      if($fotoSekarang != 'no-image.jpg'){
        unlink('assets/img/'. $fotoSekarang);
      }
...
<!-- Content -->
<div class="container">
  <h2 class="my-4 text-center">Ubah Produk</h2>
  <div class="card">
    <div class="card-body">
          ...
          <div class="col-md-6">
            ...
            <div class="row my-2">
              ...
              <div class="form-group mt-2">
                ...
                <select class="form-select" name="id_kategori_produk" required>
                  <?php
                  $queryKategori = "SELECT * FROM kategori_produk";
                  $dataKategori = ambilData($queryKategori);
                  foreach($dataKategori as $d) : ?>
                    <option <?php echo ($data['id_kategori_produk'] == $d['id_kategori_produk'] ? 'selected' : '') ?> value="<?php echo $d['id_kategori_produk'] ?>"><?php echo $d['nama_kategori'] ?></option>  
                  <?php endforeach ?>
                </select>
              </div>
              ...
              <div class="form-group mt-2">
                ...
                <input type="number" class="form-control" name="hargaBeli" value="<?php echo $data['harga_beli'] ?>" required>
              </div>

              <div class="form-group mt-2">
                <label class="form-label">Harga Jual</label>
                <input type="number" class="form-control" name="hargaJual" value="<?php echo $data['harga_jual'] ?>" required>
              </div>

              <div class="form-group mt-2">
                <label class="form-label">Berat Produk</label>
                <input type="number" class="form-control" name="berat" value="<?php echo $data['berat'] ?>" required>
              </div>
              ...
              <div class="mb-3">
                ...
                <img class="img-thumbnail" src="assets/img/<?php echo $data['foto'] ?>" width="100">
                <input type="hidden" name="fotoSekarang" value="<?php echo $data['foto'] ?>">
              </div>
